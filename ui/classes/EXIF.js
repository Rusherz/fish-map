export default class EXIF {
	...
	getImageData(img, callback) {
		if (img instanceof Image || img instanceof HTMLImageElement) {
			if (/^data\:/i.test(img.src)) {
				...
				this.handleBinaryFile(img, arrayBuffer, callback)
			} else if (/^blob\:/i.test(img.src)) {
				// Object URL
				let fileReader = new FileReader()

				fileReader.onload = function (e) {
					this.handleBinaryFile(img, e.target.result, callback)
				}
	...
	findEXIFinJPEG(file) {
		...
		if (dataView.getUint8(0) != 0xff || dataView.getUint8(1) != 0xd8) {
			if (this.debug) {
				console.debug("Not a valid JPEG")
			}
		...
		while (offset < length) {
			...
			if (marker == 225) {
				if (this.debug) {
					console.debug("Found 0xFFE1 marker")
				}
	...
	readEXIFData(file, start) {
		if (this.getStringFromDB(file, start, 4) != "Exif") {
			if (this.debug) {
				console.debug(
					"Not valid EXIF data! " +
						this.getStringFromDB(file, start, 4)
				)
			}
		...
		if (file.getUint16(tiffOffset) == 0x4949) {
			...
			if (this.debug) {
				console.debug("Not valid TIFF data! (no 0x4949 or 0x4D4D)")
			}
		...
		if (file.getUint16(tiffOffset + 2, !bigEnd) != 0x002a) {
			if (this.debug) {
				console.debug("Not valid TIFF data! (no 0x002A)")
			}
			return false
		}
